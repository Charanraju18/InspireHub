    ...
    try {
      ...
      try {
        const domainsResponse = await fetch(
          "http://localhost:5000/api/roadmaps/domains"
        );
        ...
        if (domainsResponse.ok) {
          ...
          if (Array.isArray(domainsData) && domainsData.length > 0) {
            ...
            const uniqueDomains = [
              ...new Set(roadmapsData.map((r) => r.domain).filter((d) => d)),
            ];
            setDomains(["All Categories", ...uniqueDomains]);
            console.log(
              "🔄 Used fallback domains from roadmaps:",
              uniqueDomains
            );
          }
        } else {
        ...
        console.warn(
          "⚠️ Domains API failed, using fallback:",
          domainError.message
        );
    ...
    if (selectedDomain === "All Categories") {
      ...
      const filtered = roadmaps
        .filter((roadmap) => {
          ...
          console.log(
            `🔍 Checking: "${roadmap.domain}" === "${selectedDomain}" = ${match}`
          );
          return match;
        })
        .slice(0, 6);
      ...
      console.log(
        `✅ Found ${filtered.length} roadmaps for domain: ${selectedDomain}`
      );
    }
  };
      ...
      <div className="container">
        <div className="section-heading text-center">
          ...
          <p className=" wow bounceInUp">
            Discover structured learning paths created by industry experts to
                  ...
                  >
                    <i
                      className={`text-xl d-flex ${
                        domainIcons[domain] || "ph-bold ph-squares-four"
                      }`}
                    />
                    {domain.length > 12
            ...
            <div className="row gy-4">
              {filteredRoadmaps.length > 0 ? (
                      ...
                      <div className="course-item__thumb rounded-12 overflow-hidden position-relative mb-3">
                        ...
                        >
                          <img
                            ...
                            onError={(e) => {
                              e.target.src =
                                "assets/images/thumbs/raodmapsDefault.png";
                            }}
                          />
                        </Link>
                        ...
                        {/* ✅ Difficulty badge */}
                        <span
                          className={`position-absolute top-3 end-3 badge text-white px-2 py-1 ${getDifficultyColor(
                            roadmap.difficulty
                          )}`}
                        >
                          {roadmap.difficulty}
                ...
                <div className="col-12 text-center py-5">
                  ...
                  <h5 className="text-neutral-500 mb-3">
                    No roadmaps found for "{selectedDomain}"
                  </h5>
